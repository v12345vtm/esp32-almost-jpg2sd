...
//https://github.com/v12345vtm/ESP32-CAM-IPCAM     https://forum.arduino.cc/index.php?topic=412932.0
...
<title>v4jpg almost jpg  v12345vtm ESP32-CAM-IPCAM</title>
<style>
...
void stop_streamwebserver() //stop de foto server vanuit de ino-file
 ...
 // size_t is an unsigned integer type of at least 16 bit 
    ...
    if(!index){
         Serial.println("jpg_encode_stream index=fixed 12");
    Serial.println((const char *)index);
        j->len = 0;
    }
    if(httpd_resp_send_chunk(j->req, (const char *)data, len) != ESP_OK){
                     countchuncks = countchuncks+1;
               //   send jpg chuncks -parts until webpage is loaded
                return 0; //almost 300 à 400 times we do pass here
    }
    j->len += len;
   // Serial.println("jpg_encode_stream jpgencode=");
    Serial.println((const char *)data); //print parts of the JPG stream ! 
    return len;  //512 meestal
}
 ...
 */
 ...
 countchuncks =0 ; //reset test
res = frame2jpg_cb(  fb,12,  jpg_encode_stream, &jchunk);
Serial.println(res);// is 1 if jpg convertion worked

Serial.printf("JPG: %uB %ums\n", (uint32_t)(fb_len), (uint32_t)((fr_end - fr_start) / 1000));
Serial.println("aantal chunck= " + String(countchuncks));// 
return res; 
}
